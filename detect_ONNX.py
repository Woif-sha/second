...
class YOLOV5_ONNX(object):
            ...
            if det is not None and len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if int(c) == 0:
                        ...
                        ans['黑色'] = n
                    num += n  # 人数求和
                s = f"A区域有友军{ans.get('红色')}人，敌人{ans.get('蓝色')}人，人质{ans.get('黑色')}人。"  # 人群统计结果
                print(s)
    ...
    def draw(self, img, boxinfo):
        ...
        cv2.imwrite(f"region{region[0]}.jpg", img)
        #im0 = cv2.resize(img, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR)  # 缩放图片到原来的一半
        #cv2.imshow('result', im0)
        #cv2.waitKey(0)


if __name__ == "__main__":
    model = YOLOV5_ONNX(onnx_path=r"E:\PyCharm Project\CrowdRecognition\weights\yolov5s_cartoon.onnx")
    model.infer(img_path=r"E:\PyCharm Project\CrowdRecognition\images\TZ3_No9.jpg")
